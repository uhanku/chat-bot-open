import Link from "next/link";
import { listChats } from "@/lib/database";
import { calculateOpenAIChatPrice, isOpenAIModel } from "@/lib/pricing";

export const dynamic = "force-dynamic";

function formatDate(value: Date) {
  return new Intl.DateTimeFormat("en-GB", {
    dateStyle: "medium",
    timeStyle: "short",
  }).format(value);
}

function empty(value: string | null) {
  return value?.trim() || "None";
}

function formatCost(inputSize: string, outputSize: string) {
  const model = process.env.API_MODEL;

  if (!isOpenAIModel(model)) {
    return "Unavailable";
  }

  return new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD",
    minimumFractionDigits: 6,
    maximumFractionDigits: 6,
  }).format(
    calculateOpenAIChatPrice(model, Number(inputSize), Number(outputSize)),
  );
}

export default async function ChatsPage() {
  const chats = await listChats();

  return (
    <main className="flex-1 bg-[#f7f5f0] px-6 py-8 font-sans text-[#1f1b16]">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <header className="flex flex-wrap items-end justify-between gap-4 border-b border-[#d8d0c2] pb-5">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.12em] text-[#746957]">
              Database
            </p>
            <h1 className="mt-1 text-3xl font-semibold">Chats</h1>
          </div>
        </header>

        <div className="overflow-x-auto border border-[#d8d0c2] bg-white">
          <table className="min-w-full border-collapse text-left text-sm">
            <thead className="bg-[#ece7dc] text-xs uppercase tracking-[0.08em] text-[#5d5446]">
              <tr>
                <th className="px-4 py-3 font-semibold">Chat</th>
                <th className="px-4 py-3 font-semibold">Status</th>
                <th className="px-4 py-3 font-semibold">Lead</th>
                <th className="px-4 py-3 font-semibold">Score</th>
                <th className="px-4 py-3 font-semibold">Messages</th>
                <th className="px-4 py-3 font-semibold">Input</th>
                <th className="px-4 py-3 font-semibold">Output</th>
                <th className="px-4 py-3 font-semibold">Cost</th>
                <th className="px-4 py-3 font-semibold">Updated</th>
                <th className="px-4 py-3 font-semibold">Created</th>
              </tr>
            </thead>
            <tbody>
              {chats.map((chat) => (
                <tr
                  className="border-t border-[#ebe4d8] align-top hover:bg-[#fbfaf7]"
                  key={chat.id}
                >
                  <td className="max-w-[260px] px-4 py-3">
                    <Link
                      className="font-semibold text-[#6b4a12] underline underline-offset-2"
                      href={`/chats/${chat.id}`}
                    >
                      {empty(chat.chat_name)}
                    </Link>
                    <div className="mt-1 break-all font-mono text-xs text-[#746957]">
                      {chat.id}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="rounded-sm border border-[#d8d0c2] px-2 py-1 text-xs font-semibold">
                      {chat.is_active ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="max-w-[240px] px-4 py-3">
                    <div className="font-medium">{empty(chat.client_name)}</div>
                    <div className="mt-1 break-words text-xs text-[#746957]">
                      {empty(chat.email_address)}
                    </div>
                  </td>
                  <td className="px-4 py-3 font-semibold">{chat.score}</td>
                  <td className="px-4 py-3">{chat.message_count}</td>
                  <td className="px-4 py-3">{chat.input_size}</td>
                  <td className="px-4 py-3">{chat.output_size}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {formatCost(chat.input_size, chat.output_size)}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {formatDate(chat.updated_at)}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {formatDate(chat.created_at)}
                  </td>
                </tr>
              ))}

              {chats.length === 0 ? (
                <tr>
                  <td
                    className="px-4 py-8 text-center text-[#746957]"
                    colSpan={10}
                  >
                    No chats found.
                  </td>
                </tr>
              ) : null}
            </tbody>
          </table>
        </div>
      </div>
    </main>
  );
}
